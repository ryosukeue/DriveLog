...
struct StartMonitoringUseCaseTests {
    @Test("starts SLC storage and all providers once")
    ...
    @Test("motion denial does not stop SLC or visit")
    ...
    @Test("visit failure does not stop SLC or motion")
    ...
    @Test("location failure stops storage subscription and skips auxiliaries")
    ...
    func activeProviders() async throws {
        ...
        #expect(fixture.location.appliedModes().isEmpty)
        #expect(fixture.motion.callCounts().start == 0)
        #expect(fixture.visit.callCounts().start == 0)
        #expect(fixture.logger.records.isEmpty)
        #expect(await fixture.storageCoordinator.isRunning())
